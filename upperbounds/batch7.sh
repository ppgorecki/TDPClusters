...
function Usage()
{
    ...
    -F FGREEDYSRCDIR - directory with fgreedy.c source (default is .)
    ...
    -c "SIDE,C1,C2,C3,LIMIT" - create cube_s*.tsv for estimating heuristic errors and initialize; 
                               note that index.tsv is updated therefore previous results may be invalid
        SIDE - generated small cubes of size 1,2,..,SIDE (kval=SIDE^3)
    ...
    -C DIR - analyze heuristic errors based on DIR/stats.tsv; seaborn package required to draw (see $DIR/cubetestplot.py)
  ...
  Usage:    
    # enter TDPClusters/upperbounds
    $0 -i destdir             # initalize directory with data subdir
    cd $destdir    
    $0 -E work50 -N50         # execute with N50
    $0 -E work6 -N6           # execute with N6
    $0 -m work*               # merge to release 

  Cubetest:
    # enter TDPClusters/upperbounds
    $0 -i $destdir            # initalize directory; data subdir (maybe empty) should be present
    cd $destdir    
    $0 -c "5,5,5,5,3000"      # prep some small cubetest
    $0 -i .                   # reinitilize 
    $0 -E work6 -N6           # execute with -N6
    $0 -m work*               # merge to release 
    $0 -C release*            # make error report from cubedata
...
set -- `getopt i:e:j:N:T:v:E:x:R:mfp:P:t:Sc:C:uV:M:F: $*`
...
then
  ...
  # Locate fgreedy
  echo Searching for fgreedy
  FGREEDY=
  SCRDIR=$( dirname $0 )
  if [[ -d $INITDIR ]] && [[ -f $INITDIR/fgreedy ]]
  then
    FGREEDY=$INITDIR/fgreedy
  elif [[ -f $FGRSRC/fgreedy ]]
  then
    FGREEDY=$FGRSRC/fgreedy
  elif [[ -f fgreedy ]]
  then
    FGREEDY=fgreedy
  elif [[ -f $SCRDIR/fgreedy ]]
  then
    FGREEDY=$SCRDIR/fgreedy
  elif [[ -f $FGRSRC/fgreedy.c ]]
  then     
    cd "$FGRSRC"
    gcc -std=gnu99 -O3 -o fgreedy fgreedy.c pp.c tools.c
    cd -
    FGREEDY=$FGRSRC/fgreedy
    exit -1
  else
    echo "$FGRSRC directory has no fgreedy sources"      
    echo "Use -F option to set the directory."
    exit -1
  fi
  ...
  if [[ -f $FGREEDY ]]
  then
    echo "fgreedy found: $FGREEDY"
  else
    echo "Cannot initialize fgreedy"
    exit -1
  fi
  ...
  if ! [ -f fgreedy ]; then cp $FGREEDY . ; fi
  ...
  echo "Now enter the project directory" $(realpath .) "and run" $(basename $0)
...
EOF
  ...
  echo "Cubetest prepared; run $0 -i . to reinitialize"
...
then
  ...
  FGREEDY=$( realpath fgreedy )
  [[ -f $FGREEDY ]] || { echo No $FGREEDY file; exit -1; }
